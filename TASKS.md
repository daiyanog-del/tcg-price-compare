...
- [ ] **★X自動投稿の再開（2026-08-07目安）** — 第3弾（検索語正規化の是正）も 2026-08-03 にデプロイしたため、第2弾と同じ起点で目安日は据え置き。検索語修正のデプロイに伴い post-x.yml の schedule を停止中。`collect_prices` が4回走り、get_price_movers の比較窓(3日)から変更前の価格が抜けたら schedule のコメントアウトを戻す。戻す前に /api/movers の上位に「サイバー」「いたずら好きな双子悪魔」等の見かけ急落が残っていないか確認する（経緯は docs/decisions.md 2026-08-03）
...
- [x] 2026-08-03 **店舗検索の取りこぼし修正 第2弾（カードラッシュ）** — 照合 `_build_flex_pattern`/`_FLEX_SEP` の区切り集合を `_FLEX_SEP_CHARS` に集約し実測で仕分けた記号を追加（区切り=「」『』☆★“”〜～＠＆！．／× ／ 文字のまま=ギリシャ文字）。カードラッシュ専用の検索語生成を新設し、販売にページ送りを追加。効果は販売301→422件・買取187→220件、他8経路は増減ゼロ。コミット bdfc702（進行中欄に残っていたのを2026-08-03に完了へ移動）
