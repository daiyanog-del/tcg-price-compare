...
- [ ] **★X自動投稿の再開（2026-08-07目安 → 第3弾デプロイ日+4日へ再設定）** — 検索語正規化の是正（2026-08-03 第3弾）でさらに新規取得が増えるため、目安日はそのデプロイ日を起点に取り直すこと。検索語修正のデプロイに伴い post-x.yml の schedule を停止中。`collect_prices` が4回走り、get_price_movers の比較窓(3日)から変更前の価格が抜けたら schedule のコメントアウトを戻す。戻す前に /api/movers の上位に「サイバー」「いたずら好きな双子悪魔」等の見かけ急落が残っていないか確認する（経緯は docs/decisions.md 2026-08-03）
...
- [x] 2026-08-03 **検索語正規化の不揃いを是正（空白圧縮＋コロン除去の共通化）** — `_squeeze_spaces()` を新設し `_normalize_search_query` / `_cardrush_search_query` の両方で使用。コロンはカードラッシュ専用ではなく共通側で落とす形に統一。実測: 空白圧縮は影響101件×5経路でカードラボ販売 124→133件・**減少0**、コロン除去は影響9件×7経路で**カードラッシュ買取 0→27件・遊々亭買取 6→29件・遊々亭販売 7→16件**・カードラボ買取 17→18件・**減少0**。トレコロは対象外（全15,522件で余分な空白が発生しないことを確認）。pytest 219件全緑。詳細 decisions.md 2026-08-03
