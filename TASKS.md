...
- [ ] **X新形式（bigmove）の撤退ゲート判定** — **2026-09-15頃**: bigmove投稿のインプレッション中央値100未満かつGA4でX流入なしなら凍結（docs/decisions.md 参照）。それまで毎日 JST 20:07 に自動実行、たまに投稿内容を目視確認
- [ ] **急落投稿の誤認観察** — 状態難あり品の安値出品を「急落」と誤認するリスクが既知。実験期間中に誤認投稿があれば対策（複数店舗確認等）を検討
