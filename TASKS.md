...
- [ ] **値動きランキングの2系統併存の解消検討** — Web表示=Supabase RPC / X投稿=Python集計で結果が食い違いうる。集計窓も days=2（app.py販売）と days=3（買取・X側）で不一致。調査済み（2026-07-10）: featured投稿の allowed_names 絞り込みはRPCのtop_nに乗らないため完全一本化は不可。選択肢はユーザー判断待ち（docs/activeContext.md 参照）
