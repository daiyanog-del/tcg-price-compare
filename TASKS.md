...
- [ ] **GA4 の測定ID発行と本番設定** — コード側は導入済み（GA_MEASUREMENT_ID 環境変数駆動）。ユーザーが GA4 プロパティを作成 → 測定ID（G-…）を Render の環境変数に設定すれば計測開始
...
- [x] 2026-07-10 GA4計測タグを導入（環境変数駆動・index/solitaire対象・pytest+実レンダリング検証済み）
