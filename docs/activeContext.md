...
- **X新形式（bigmove）は実装済み・GitHub Actions実環境でドライラン検証済み（2026-07-10、選定=ブラックマジシャン/プリシク +489%）・毎日 JST 20:07 実行から適用**。観察事項: 投稿内容の目視（特に急落側の状態難あり品誤認）。撤退ゲート判定 2026-09-15頃（インプ中央値100未満かつX流入なしなら凍結、decisions.md）
