...
- **⚠ X自動投稿は schedule 停止中（2026-07-10）**。ドライランの選定結果（ブラックマジシャン +489%）が**偽陽性と確定**したため（中黒表記ゆれによる系列分裂＋単一店舗の在庫入れ替わり。decisions.md 参照）。実装（post_big_movers）自体は完成・テスト済みだが、騰落率パイプラインの前提監査（別セッションで実施中）と対策が終わるまで再開しない。撤退ゲート判定（2026-09-15頃）も監査後に再設定
