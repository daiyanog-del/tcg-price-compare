...
- 価格変動の色は販売と買取で意味が逆になる。共通CSSを変更するときは、買取上昇＝緑・買取下落＝赤が詳細度で上書きされていないか、両方向を実画面で確認する（2026-09-25 レビュー指摘）。
...
## 画面デザイン
- 価格比較トップに宣伝用のキャッチコピーや操作を重複説明する見出しを追加しない。検索と価格情報を優先する（2026-09-25 ユーザー指摘）。
- 共通アクセント色を明色へ変更するときは、白文字のボタン・横向き案内・バッジなど背景色に変数を使う箇所のコントラストも確認する（2026-09-26 レビュー指摘）。
- 枠線の整理は指摘箇所だけでなく同じ画面の同用途の領域・通常操作も点検する。領域の識別は背景を優先し、入力・選択・録画・エラーなど状態の目印と区別する（2026-09-26 ユーザー指摘）。
