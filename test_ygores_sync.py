...
MIRROR_PENDULUM = {"id": 11213, "type": "monster", "name": "オッドアイズ・ペンデュラム・ドラゴン",
                   "englishAttribute": "dark", "level": 7, "atk": 2500, "def": 2000,
                   "pendScale": 4, "pendEffect": "①：自分のPモンスターの戦闘で発生する自分への戦闘ダメージを０にできる。",
                   "effectText": "①：このカードが相手モンスターとの戦闘で相手に与える戦闘ダメージは倍になる。",
                   "properties": ["ドラゴン族", "ペンデュラム", "効果"]}
...
def test_pendulum_scale_and_effect():
    """ペンデュラム: ミラー形式(pendScale/pendEffect)・API形式(pendulumScale/pendulumEffectText)
    の両方からスケールとペンデュラム効果を取り出す。非ペンデュラムは scale=None・effect 空。"""
    # ミラー形式
    pend = _summ(MIRROR_PENDULUM)
    assert pend["pendulum_scale"] == 4
    assert "戦闘ダメージ" in pend["pendulum_effect"]
    assert pend["effect_text"] != pend["pendulum_effect"]   # 本体効果と別物
    assert pend["race_ja"] == "ドラゴン族"

    # API形式
    api_pend = {"cardData": {"ja": {"name": "オッドアイズ", "cardType": "monster",
                                    "properties": [], "atk": 2500, "level": 7,
                                    "pendulumScale": 4, "pendulumEffectText": "P効果テキスト"}}}
    s = CardDataRepository._summarize(11213, api_pend)
    assert s["pendulum_scale"] == 4
    assert s["pendulum_effect"] == "P効果テキスト"

    # 非ペンデュラム（通常モンスター）はスケールなし・効果空
    normal = _summ(MIRROR_NORMAL_MON)
    assert normal["pendulum_scale"] is None
    assert normal["pendulum_effect"] == ""
