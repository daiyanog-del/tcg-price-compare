...
遊戯王OCGカードの販売・買取価格をリアルタイムで比較するWebサービスです。
...
| 機能 | 説明 |
|------|------|
| 🔍 複数店舗同時検索 | 販売7店舗（遊々亭・カードラッシュ・トレコロCB・カーナベル・カードラボ・まんぞく屋・駿河屋）に対応。デフォルト検索は駿河屋を除く6店を同時スクレイピング |
...
主要ファイルの詳細は [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) を参照してください。
...
|------|------|-----------|
| `PORT` | リッスンポート | `5000` |
| `FLASK_DEBUG` | デバッグモード（既定でフェイルクローズ。ローカル開発時は`1`を明示設定） | `0` |
