...
async function _fetchAndShow(name, imgSrc) {
  ...
  try {
    ...
    console.log('[price-debug]', { sentName: name, resolvedName: priceData?.card_name, dataLen: priceData?.data?.length, debug: priceData?.debug, url: `${API_PRICE_HISTORY}?card=${encodeURIComponent(name)}` });
    if (data.found) {
